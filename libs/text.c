...
void print_hd_5x3_at (int x,int y, char *text, unsigned char brightness)
{
	while (*text)
	{
		putc_hd_5x3_at(x,y,*text,brightness);
		x+=4;
		text++;
	}
...
void putc_hd_5x3_at (int x,int y, char text, unsigned char brightness)
{
	text -= 32;
	...
	for (i = 0; i < 3; i++)
	{
		...
		for (j = 0; j < 5; j++)
		{
			if(ch & (1<<j))
			{
				set_frame_buffer_hd(x+i,y-j+4,brightness);
			}
			else
			{
				set_frame_buffer_hd(x+i,y-j+4,0);
			}
		}
	}
}

void print_hd_unsigned_5x3_at(int x, int y, unsigned int number, int length, char pad, unsigned char brightness) {
	x += length * 4 - 1;
	int i;
	for(i = 0; i < length; i++) {
		int d = number % 10;
...
void print_hd_num_5x3_at (int x, int y, int number, int length, int pad, unsigned char brightness)
{
	...
	if (length < len) {
		int i;
		for (i = 0; i < length; i++) {
			putc_hd_5x3_at (x += 4, y, '*', brightness);
		}
		return;
	}
	int i;
	for (i = 0; i < length - len; i++) {
		putc_hd_5x3_at (x += 4, y, pad, brightness);
	}
	print_hd_5x3_at(x, y, (char*)s, brightness);
